...
const SignIn = () => {
    ...
    const {pageOriginURL, expressServerURL, 
        noUserIdSoRedirectToSigninPage, setNoUserIdSoRedirectToSigninPage
    } = useGlobal();
...
const responseGoogleSignUpButton = async (response) => {
    try{
        ...
        let requestToExpress = await axios.post(`${expressServerURL}/sign-in-user`, {
        ...
        if(respondsFromExpress === 'googleId-did-not-match'){
            alert('google ID did NOT match with the email, please check your google address, email and security');
            return;
        }
...
// *******************************************************************
    return (
        ...
        <form className='registrationPageFormBox px-md-3 pt-5 p-md-5' style={{maxWidth: '600px'}}>
...
<div className='px-4 py-3 d-flex justify-content-center align-items-center' style={{background: '#f8d7da', borderRadius: '5px', border: '1px solid #f5c6cb'}}>

    <div className='signinOrSignupWarningText text-center p-0 m-0'>You need to sign in or sign up before continuing.</div>
...
{/* start of Google Login Wrapper Box */}

<div className="d-flex flex-column justify-content-center align-items-center" onClick={()=> 
    {
...
{googleUserNotFound && 
        <div className='registrationPageWarningText pt-1'>This user not found on our system, please register&nbsp;
        <Link href='/register'>
...
{/* end of Google login wrapper that holds everything in google Sign in box */}
</div>
...
<div className="d-flex flex-column">
    ...
    <input type="email" id='email' className='registrationFormInput p-2' value={email} onChange={e=> {
        setEmail(e.target.value); setEmailWarning(false); setPleaseEnterValidEmailWithAtSymbolWarning(false); 
        setNoSuchUserFound(false); 
        setGoogleUserNotFound(false);
        setNoUserIdSoRedirectToSigninPage(false);
    }} placeholder='Enter' style={/@/g.test(email) ? {border: '1px solid #28a745', width: '100%'} : {width: '100%'}} />
    ...
    {noSuchUserFound && 
        <div className='registrationPageWarningText pt-1'>Email not found, please register&nbsp;
        <Link href='/register'>
            <a className='warningLinkForEmailPleaseRegister'>
                <u>here</u>
            </a>
...
<div style={{position: 'relative', zIndex: '100'}}>
         ...
         <span className='showHideButtonOnInputInRegistration' style={{marginLeft: 'auto', position: 'absolute', right: '45px', top: '29px', cursor: 'pointer'}} onClick={()=> 
         {
             setMainPasswordFieldTypeSetToPassword(!mainPasswordFieldTypeSetToPassword);
         }
            }>
...
placeholder='Enter a password' style={{width: '100%'}}/>
...
{wrongPassword && 
<div className='registrationPageWarningText pt-1'>Incorrect password entered, please check password and try again</div>
}  
...
<button type='button' className='loginFormLoginButton mb-3 mb-md-0' onClick={handleSubmit}>Log In</button> 
